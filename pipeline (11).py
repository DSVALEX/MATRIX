...
def run_pipeline_from_parsed(parsed, country, output_dir, cfg,
    ...
    if has_pallet:
        ...
        # rows and vice-versa — different shipment profiles), write numerically.
        ...
        write_matrix_numeric(ext_all, ext_path, cfg, vl)
        write_matrix_numeric(opt_all, opt_path, cfg, vl)
        write_matrix_numeric(min_all, min_path, cfg, vl)
...
def write_combined_matrix(frames, output_path, variables_layout=None):
    """Merge every country's minimal numeric frame into ONE sheet, sorted by
    country then price. Values written numeric so per-country surcharges stay
    correct. `frames` is a list of DataFrames (from result['minimal_df'])."""
