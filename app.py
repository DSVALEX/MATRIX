...
def variables_layout(fuel_vals, maut_dhl, maut_dpd, pallet_vals=None):
    ...
    return [
        ...
        (None, None),                                         # B10
        ('FUEL DHL PALLET', pv.get('fuel',     0.15)),        # B11
...
def make_combined(results, variables_layout_rows):
    """Build ONE workbook with every country's minimal matrix in a single sheet.

    Reads each country's persisted numeric minimal frame and concatenates them
    via pipeline.write_combined_matrix. Values are written numeric (not formulas)
    so per-country surcharges (MAUT, pallet toll/admin) stay correct in one file.
    Returns the .xlsx bytes, or None if no frames are available.
    """
    frames = []
    for country, r in results.items():
        p = r.get('minimal_df')
    ...
    if pallet_uploaded is not None:
        ...
        if 'pallets' not in st.session_state:
            with st.spinner("Reading pallet rate card…"):
                st.session_state['pallets'] = pp.parse_pallet_rate_card(
    ...
    if st.session_state.get('pallets'):
        ...
        with cols[0]:
            pallet_vals['toll'] = st.number_input(
                ...
                value=float(_pd['toll_pct']), step=0.0001, format="%.4f", key='pal_toll')
        with cols[1]:
        ...
        pallet_vals['factor'] = st.number_input(
            'Factor (divides pallet rate)', min_value=0.0001,
...
if run_btn and uploaded and selected:
    ...
    # Apply sidebar pallet surcharge edits to the pallet carrier defaults so the
    # numeric column values match the Variables-sheet formulas.
    ...
    if pallet_vals:
        ...
        # Sidebar toll/admin apply to the toll-bearing countries (UK by default).
        for _iso in pal_overrides:
            pal_overrides[_iso] = {
                'toll_pct':           pallet_vals.get('toll',  pal_overrides[_iso].get('toll_pct', 0.0)),
                'admin_per_shipment': pallet_vals.get('admin', pal_overrides[_iso].get('admin_per_shipment', 0.0)),
            }
    # Remember the Variables layout for the combined export (reference sheet).
            ...
            if is_master:
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    result = pl.run_pipeline_from_parsed(parsed, country, tmp, cfg, cd, vl,
                                                         ...
                                                         pallet_defaults=pal_defaults,
                                                         pallet_overrides=pal_overrides)
...
if st.session_state.results:
    ...
    with dc2:
        ...
        _combined = make_combined(results, _vl_rows)
